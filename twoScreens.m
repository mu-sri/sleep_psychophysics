...
%subj.nchannel = input('Enter number of channels (1 or 3):\n');
...
Dataname = strcat('ccshs_1800',num2str(whichData,'%03d'),'_3EXG/');
...
% ZHAO - Remove me
Exp.Cfg.screenNumber1 = max(Exp.Cfg.screens)
Exp.Cfg.screenNumber2 = max(Exp.Cfg.screens)-1

Exp.Cfg.Win1Size_ori = round(Screen('Rect', Exp.Cfg.screenNumber1));
Exp.Cfg.Win1Size = Exp.Cfg.Win1Size_ori;
Exp.Cfg.Win1Size(3) = Exp.Cfg.Win1Size(3)*0.95; % Did this to make room for 

Exp.Cfg.Win2Size_ori = round(Screen('Rect', Exp.Cfg.screenNumber2));
Exp.Cfg.Win2Size = Exp.Cfg.Win2Size_ori;
Exp.Cfg.Win2Size(3) = Exp.Cfg.Win2Size(3)*0.95; % Did this to make room for 

%MP = get(0, 'MonitorPositions');
%Exp.Cfg.WinSize_ori = round(get(0, 'Screensize')); % Get rid of 2/3 later for full screen
%Exp.Cfg.WinSize_ori = round(MP(Exp.Cfg.screenNumber,:)); % Get rid of 2/3 later for full screen
%Exp.Cfg.WinSize = Exp.Cfg.WinSize_ori;
% Exp.Cfg.WinSize(3) = Exp.Cfg.WinSize(3)*0.95; % Did this to make room for 
    % confidence level colour legend bar; it was a last-min addition and 
    % didn't want to change a bunch of things (wanted to keep 1:1 ratio 
...
% Define colours
Exp.Cfg.Color.white = WhiteIndex(Exp.Cfg.screenNumber1); % Define white colour
Exp.Cfg.Color.black = BlackIndex(Exp.Cfg.screenNumber1); % Define black colour
...
[Exp.Cfg.win1, Exp.Cfg.windowRect1] = Screen('OpenWindow', ...
	Exp.Cfg.screenNumber1 , Exp.Cfg.Color.gray, []);

% Find window size
[Exp.Cfg.width1, Exp.Cfg.height1] = Screen('WindowSize', Exp.Cfg.win1);

[Exp.Cfg.win2, Exp.Cfg.windowRect2] = Screen('OpenWindow', ...
	Exp.Cfg.screenNumber2 , Exp.Cfg.Color.gray, []);

% Find window size
[Exp.Cfg.width2, Exp.Cfg.height2] = Screen('WindowSize', Exp.Cfg.win2);

% Font
Screen('TextFont', Exp.Cfg.win1, 'Arial');
Screen('TextFont', Exp.Cfg.win2, 'Arial');

% Blending mode (for transparency effect - for highlighting)
Screen('BlendFunction', Exp.Cfg.win1, GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
Screen('BlendFunction', Exp.Cfg.win2, GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);

win1Hor = Exp.Cfg.windowRect1(3)
win1Vert = Exp.Cfg.windowRect1(4)
win2Hor = Exp.Cfg.windowRect2(3)
win2Vert = Exp.Cfg.windowRect2(4)
...
Exp.Cfg.rs = (win2Hor*1.7/5)*1.75; % Outermost diameter
...
buttonOffset_x = win2Hor/2; % Centre of the right half
buttonOffset_y = win2Vert*0.55; % Mid-height
...
Screen('FillRect',  Exp.Cfg.win1, backgroundColour); % Fill the whole window rectangle
Screen('FillRect',  Exp.Cfg.win2, backgroundColour); % Fill the whole window rectangle
...
for i = 1:nPartition-1
    ...
    Screen('FillPoly', Exp.Cfg.win2, pentColour(i), ...
...
Screen('FillPoly', Exp.Cfg.win2, backgroundColour, ...
...
for i = 1:5
    % Pentagon outliine
    for j = 1:nPartition
        Screen('DrawLine', Exp.Cfg.win2,Exp.Cfg.Color.black, pentCoord_x(j,i), ...
            pentCoord_y(j,i), pentCoord_x(j,i+1), pentCoord_y(j,i+1), lineWidth)
    end
    
    % Lines across pentagons
    Screen('DrawLine', Exp.Cfg.win2,Exp.Cfg.Color.black, pentCoord_x(1,i), ...
...
Screen('TextSize',Exp.Cfg.win2, floor((Exp.Cfg.Win2Size_ori(3)-...
    Exp.Cfg.Win2Size(3))*0.4)); % Sleep class texts size
sleepClassTexts = {'Wake', 'N1', 'N2', 'N3', 'REM'}; % Sleep class texts
for i = 1:5 % 5 because there are 5 classes (pentagon)
	DrawFormattedText(Exp.Cfg.win2, sleepClassTexts{i}, ... % Put texts
...
legendPos_x = (Exp.Cfg.Win2Size_ori(3)-Exp.Cfg.Win2Size(3))*0.1+Exp.Cfg.Win2Size(3); 
legendPos_y = Exp.Cfg.Win2Size(4)*0.1;

% Use 80% of the window height for the legend bar
barLength = Exp.Cfg.Win2Size(4)*0.8; 

% Use 80% of the width allocated (5% of window size) for the legend bar 
barWidth = (Exp.Cfg.Win2Size_ori(3)-Exp.Cfg.Win2Size(3))*0.8; 


for i = 1:nPartition-1
    % Divide up the legend bar and fill with confidence level colorus
    Screen('FillRect', Exp.Cfg.win2, pentColour(i), ... 
    ...
    DrawFormattedText(Exp.Cfg.win2, int2str(nPartition-i), ... 
        legendPos_x+barWidth/3, legendPos_y+barLength*((i-0.3)/...
        (nPartition-1)), [0 0 0]);
end

% Texts above and below; 'Sure' and 'Not sure' //may need to fix
Screen('TextSize', Exp.Cfg.win2, floor(barWidth/2));
DrawFormattedText(Exp.Cfg.win2, 'Sure', legendPos_x, ...
    legendPos_y - 5*barWidth/8, [0 0 0]);
DrawFormattedText(Exp.Cfg.win2, 'Not\nsure', legendPos_x, ...
...
for i = 1:nPartition
    confidenceMask{i} = poly2mask(pentCoord_x(i,:), pentCoord_y(i,:), ...
        Exp.Cfg.Win2Size_ori(4),Exp.Cfg.Win2Size_ori(3));
end
for i = 1:nPartition-1
...
%image_rect = [0, 0, win1Hor*19/20, floor(win1Hor*0.25)]; % Size of object images; ratio ok??
image_rect = [0, 0, win1Hor*19/20, win1Vert*19/20]; % Size of object images; ratio ok??
subjectResponse = [];
...
for m = 1:nFiles
    ...
    Probe_Tex = Screen('MakeTexture', Exp.Cfg.win1, showImage);

    % Image position; centre of the left half
	imageOffset_x = win1Hor*0.5;
	imageOffset_y = win1Vert*0.5;
    ...
    if m==1 % From the second image, we want ot update images a bit later
        ...
        Screen('DrawTextures', Exp.Cfg.win1, Probe_Tex, [], showImageProbe, 0);
        
        % Display trial label
        if ~isempty(trialLabelRect)
            Screen('FillRect', Exp.Cfg.win2, backgroundColour, trialLabelRect);    
        end

        [nx, ny, trialLabelRect] = DrawFormattedText(Exp.Cfg.win2, ['Trial ' int2str(m) ' of ' int2str(nFiles)], ...
            win2Hor*0.75, win2Vert*0.95, [0 0 0]);
        
        % Present everything
        Screen('Flip',Exp.Cfg.win1, [], 1);
        Screen('Flip',Exp.Cfg.win2, [], 1);

        SetMouse(Exp.Cfg.width2/2, Exp.Cfg.height2/2, Exp.Cfg.win2);
        ShowCursor('Hand');
    ...
    if m>1
        ...
        for i = 1:5 % 5 because there are 5 classes (pentagon)
            DrawFormattedText(Exp.Cfg.win2, sleepClassTexts{i}, ...
                sleepClassTextPos_x(i), sleepClassTextPos_y(i), [0 0 0]);
        end
        ...
        % Display trial label
        if ~isempty(trialLabelRect)
            Screen('FillRect', Exp.Cfg.win2, backgroundColour, trialLabelRect+1);    
        end
...
% ########################### make a pentagon layout func + call it
        ...
        % Draw correct images to screen
        Screen('DrawTextures', Exp.Cfg.win2, Probe_Tex, [], showImageProbe, 0);
        % Put sleep class text
        Screen('TextSize',Exp.Cfg.win2, floor(barWidth/2));
...
Screen('FillRect',  Exp.Cfg.win1, backgroundColour);
DrawFormattedText(Exp.Cfg.win1, 'Done :)', ...
				win1Hor/2, win1Vert/2, [0 0 0]);
Screen('FillRect',  Exp.Cfg.win2, backgroundColour);
DrawFormattedText(Exp.Cfg.win2, 'Done :)', ...
				win2Hor/2, win2Vert/2, [0 0 0]);
Screen('Flip',Exp.Cfg.win1, [], 1);
Screen('Flip',Exp.Cfg.win2, [], 1);
WaitSecs(1);
sca;
